...
public class CampeonatoDAOImpl implements CampeonatoDAO
{
	...
	//funcion auxiliar que permite saber si debemos crear un nuevo campeonato autom�ticamente.
	@SuppressWarnings("unchecked")
	...
	public void premiarGanadores(String nomCampeonato)
	{
		...
		// Asigno 15, 10 y 5 puntos a cada posici�n
		...
		while (it.hasNext() && (cant < 3))
		{
			...
			// Adem�s restablezco el puntaje del equipo a -1. 
	        e.setPuntaje(-1);
	...
	public DataListaGanadoresCamp listarGanadoresCampeonatos()
	{
		...
		for (Campeonato c : campeonatos)
		{
			...
			// Si el campeonato est� completo
			if(c.getCantEquipos() == c.getEquipos().size())
			{
				Date fecha_ultimo_partido = partido.getFechaPartido();
				// Si la fecha del �ltimo partido del campeonato es anterior a hoy => El campeonato finaliz�
